...
function AgentInquiries() {
  ...
  const { data, isLoading, error } = useInquiries({ scope: "agent" });
  const update = useUpdateInquiryStatus();

  // Debug logging
  console.log("🔍 Agent Inquiries Page - User:", user?.id);
  console.log("🔍 Agent Inquiries Page - Role:", role);
  console.log("🔍 Agent Inquiries Page - Data:", data);
  console.log("🔍 Agent Inquiries Page - Error:", error);
  console.log("🔍 Agent Inquiries Page - Loading:", isLoading);
  ...
  if (error) {
    return (
      <div className="mx-auto max-w-7xl px-4 py-20 text-center">
        <h1 className="text-xl font-bold text-destructive">Error loading inquiries</h1>
  ...
  const items = data ?? [];
  const unread = items.filter((i: any) => i.status === "unread").length;
            ...
            return (
              <div
                ...
                className={`rounded-2xl border bg-card p-5 transition-all ${
                  inq.status === "unread" ? "border-primary/30 bg-primary/2 shadow-sm" : ""
                }`}
              >
                <div className="flex flex-wrap gap-4 items-start">
                  ...
                  <div className="flex flex-col gap-2 shrink-0">
                    {sender?.email && (
                      <a
                        href={`mailto:${sender.email}?subject=Re: ${encodeURIComponent(
                          prop?.title ?? "Your inquiry"
                        )}`}
                      >
...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    unread: "bg-yellow-500/15 text-yellow-700 border-yellow-500/30",
